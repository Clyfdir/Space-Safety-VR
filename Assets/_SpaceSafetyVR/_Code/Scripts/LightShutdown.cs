...
public class LightShutdown : MonoBehaviour
{
    ...
    // Map intensity -> duration and min/max tick intervals
    ...
    IEnumerator FlickerRendererThenOn(RenderState rs)
    {
        ...
        while (t < duration)
        {
            ...
            if (Random.value < 0.35f)
            {
                mul = 0f;
#if AK_WWISE_ADDRESSABLES || AK_WWISE || WWISE || AK_WWISE_UNITY
                if (playSoundsDuringFlicker && Random.value < 0.6f) PostRandomFlickAt(rs.r.bounds.center, 0.7f);
#endif
            }
            else
            {
                mul = Random.Range(minBrightness, 1f);
#if AK_WWISE_ADDRESSABLES || AK_WWISE || WWISE || AK_WWISE_UNITY
    ...
    // Posts event at a world position using a temp AkGameObj (auto-cleaned).
    void PostAt(AK.Wwise.Event ev, Vector3 pos, float life = 3f)
    {
        if (ev == null) return;
        var go = new GameObject("LightSFX_Wwise");
        go.transform.position = pos;
        go.AddComponent<AkGameObj>(); // registers with Wwise and updates position
        ev.Post(go);
        Destroy(go, life);
    }
    ...
    /*
    // Unity Audio fallback intentionally omitted (use Wwise above).
    */
}
