Shader "Test/FbmSun2"
{
    Properties
    {
        ...
        _ShrinkFactor ("_ShrinkFactor",      Float) = 0.8
    }
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                r.x = CalculateFbmNoise(
                                 i.positionW+ 2*q+_Time* 0.25,
                                _NoiseScale,
                                _NoiseAmplitude,
                                _NoiseStrength,
                                _NoiseFrequency);
                
                r.y = CalculateFbmNoise(  i.positionW+3*q+_Time *0.265,
                                _NoiseScale,
                                _NoiseAmplitude,
                                _NoiseStrength,
                                _NoiseFrequency);
                
                float f = CalculateFbmNoise( i.positionW+ float3(r,q.x),
                                _NoiseScale*0.5,
                                _NoiseAmplitude,
                                _NoiseStrength,
                                _NoiseFrequency);
                ...
                fres   = saturate(fres);
                float3 rgb = lerp(_Color,_Color1,r.x);
                rgb= lerp(rgb,_Color2,f);
